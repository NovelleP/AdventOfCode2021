...
def fold_matrix(matrix: list[list[str]], foldings: list[str]) -> int:
    m = re.compile('([xy])=([0-9]+)')
    rows = len(matrix)
    cols = len(matrix[0])
    ...
    return sum([1 for i in range(rows) for j in range(cols) if matrix[i][j] == '#'])


if __name__ == '__main__':

    matrix, foldings = read_input('input.txt')
    ans = fold_matrix(matrix, foldings[:1])
    print(ans)
